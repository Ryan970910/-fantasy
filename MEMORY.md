...
## 2026-09-09 Street Court 正式前端接入

- 用户确认整站街球 Demo 后，明确授权提交并部署正式版。首页、登录/注册、阵容、排名、情报与首发空状态共用黑/橙/奶油白视觉，Anton 经 next/font/local 打包。
- 球员情报新增一个中英文姓名搜索栏，选中结果更新报告；只搜索当前真实情报数据集，缺少赛程或样本时显示原因。英文展示名保留于响应，身份仍使用 NBA ID。
- 桌面阵容采用半场位置布局；排名新增已开赛球员的交互球场。保留原有 API、认证、工资帽、锁定和服务端未开赛隐藏规则。
- 本次接入通过 TypeScript、37 项 Vitest 和生产构建；已在浏览器验证桌面/手机组件、姓名搜索、阵容选择预算及排名详情，登录注册来自实际 Next 构建。受保护组件的上线前视觉测试使用隔离样例，不代表已写入或验证生产保存。
- 只读数据库阵容关联检查通过。未执行数据库写入、迁移、同步或 Cron 调整。生产部署状态以本次 GitHub/Vercel 发布检查为准。

## 2026-09-13 NBA 官方球员伤病板块

- 赛前情报新增“球员伤病”入口 `/injuries`，页面与 `/api/nba/injuries` 均要求现有登录会话。数据只读，来自 NBA 官方季节索引中最新的伤病 PDF，按表格边界解析跨行、跨页原因，保留尚未提交的球队状态。
- 按用户要求排除发布满 7 天的报告，不建立历史伤病数据库。仅在 Vercel 服务端进程内存缓存最新结果，最多 5 分钟，到期清除；上游与响应均 no-store，无 Neon 写入、持久化文件或 Cron 调整。
- 支持中文/英文球员搜索、球队与状态筛选、北京时间报告/查询时间、官方原文链接；超过 24 小时明确标为参考。缺少近期报告、抓取失败和球队待提交分别呈现。
- 本地核对官方索引当时没有最近 7 天报告；历史官方 PDF 样本仅用于解析和隔离 UI 验证，不作为生产数据。提交部署状态以 GitHub/Vercel 实际检查为准。

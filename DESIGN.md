...
- **Navigation:** 桌面与手机共用四项中文语义链接：首页、我的阵容、实时排名、赛前情报。赛前情报进入聚合页，包含预测首发和球员情报；两个子页面均保持此导航选中。桌面使用磨损红纸覆盖旧菜单区域，配 Lucide 图标、纸面选中态和 aria-current；焦点为浅色（2px），偏移（3px）。手机仅保留底部导航，顶部页头收紧。移除未实现功能的导航入口。
...
- **Injuries:** `/injuries` 延续 Retro Courtside 的旧纸阅读面，以分隔线组织球员、出场状态和伤病原因三列；手机保留姓名与状态两列，原因另占整行。中文／英文搜索与球队、状态筛选使用原生控件；状态复用现有强调色和正文色，并始终显示中英文文字。加载、读取失败、近期无报告、筛选无结果与球队待提交分别说明；未列入报告不代表可出战，缺少原因显示明确文字，不以空白暗示健康。
